// This is modified version of expected-lite to wrap it into simjit namespace instead of nonstd
// nonstd define is #undef'd in the end of file
#define nonstd simjit::nonstd
...
template <typename E> constexpr auto make_unexpected(E &&value) -> unexpected<typename std::decay<E>::type> {
...
// #if 0 && nsel_HAVE_ADDRESSOF
    ...
    public:
        ...
        template <class U, class... Args> void emplace_value(std::initializer_list<U> il, Args &&...args) {
        ...
        template <class U, class... Args> void emplace_error(std::initializer_list<U> il, Args &&...args) {
    ...
    template <typename T, typename E> class storage_t<T, E, true, true> : public storage_t_impl<T, E> {
    public:
        storage_t() = default;
        ~storage_t() = default;
    ...
    template <typename T, typename E> class storage_t<T, E, true, false> : public storage_t_impl<T, E> {
    public:
        storage_t() = default;
        ~storage_t() = default;
    ...
    template <typename T, typename E> class storage_t<T, E, false, true> : public storage_t_impl<T, E> {
    public:
        storage_t() = default;
        ~storage_t() = default;
    ...
    nsel_constexpr auto
    invoke_member_function_impl(FnT ClassT::*memfnptr, ObjectT &&obj,
    ...
    public:
        ...
        template <typename E2 = E> nsel_constexpr14 unexpected_type &operator=(unexpected_type<E2> const &other) {
            unexpected_type{other.error()}.swap(*this);
            return *this;
        }

        template <typename E2 = E> nsel_constexpr14 unexpected_type &operator=(unexpected_type<E2> &&other) {
    ...
    public:
        ...
        template <typename E> explicit unexpected_type(E error) : m_error(std::make_exception_ptr(error)) {}
    ...
    template <typename E> constexpr bool operator<(unexpected_type<E> const &x, unexpected_type<E> const &y) {
        return x.error() < y.error();
    }
    ...
    template <typename E> class nsel_NODISCARD bad_expected_access;

    /// x.x.7 bad_expected_access<void>: expected access error

    template <> class nsel_NODISCARD bad_expected_access<void> : public std::exception {
    public:
    ...
    template <typename E> class nsel_NODISCARD bad_expected_access : public bad_expected_access<void> {
    public:
        using error_type = E;
    ...
    public:
        ...
        template <typename U, typename G nsel_REQUIRES_T(std::is_constructible<T, U const &>::value
        ...
        template <typename U,
                  typename G nsel_REQUIRES_T(std::is_constructible<T, U>::value &&std::is_constructible<E, G>::value &&
                                             !std::is_constructible<T, expected<U, G> &>::value &&
                                             !std::is_constructible<T, expected<U, G> &&>::value &&
                                             !std::is_constructible<T, expected<U, G> const &>::value &&
                                             !std::is_constructible<T, expected<U, G> const &&>::value &&
        ...
        // is_trivially_destructible_v<E> is false and !bool(*this), calls unexpect.~unexpected<E>(). Remarks: If either
        // T is cv void or is_trivially_destructible_v<T> is true, and is_trivially_destructible_v<E> is true, then this
...
#if nsel_P2505R >= 4
        template <
            typename G = E nsel_REQUIRES_T(std::is_copy_constructible<E>::value &&std::is_convertible<G, E>::value)>
        nsel_constexpr error_type error_or(G &&e) const & {
            return has_value() ? static_cast<E>(std::forward<G>(e)) : contained.error();
        }

        template <
            typename G = E nsel_REQUIRES_T(std::is_move_constructible<E>::value &&std::is_convertible<G, E>::value)>
        ...
        template <typename F nsel_REQUIRES_T(std::is_constructible<error_type, error_type &>::value &&std::is_void<
    ...
    template <typename E> class nsel_NODISCARD expected<void, E> {
    private:
        template <typename, typename> friend class expected;
    ...
    public:
        ...
        void value() const {
            if (!has_value()) { error_traits<error_type>::rethrow(contained.error()); }
        }
    ...
    private:
        detail::storage_t<void, E, std::is_copy_constructible<E>::value, std::is_move_constructible<E>::value>
            contained;
    };
    ...
    template <typename T, typename E> constexpr bool operator<(expected<T, E> const &x, expected<T, E> const &y) {
    ...
    template <typename T1, typename E1, typename T2> constexpr bool operator!=(expected<T1, E1> const &x, T2 const &v) {
        return bool(x) ? *x != v : true;
    }

    template <typename T1, typename E1, typename T2> constexpr bool operator!=(T2 const &v, expected<T1, E1> const &x) {
    ...
    template <typename T, typename E> constexpr bool operator<(expected<T, E> const &x, unexpected_type<E> const &u) {
        return (!x) ? (x.get_unexpected() < u) : false;
    }

    template <typename T, typename E> constexpr bool operator<(unexpected_type<E> const &u, expected<T, E> const &x) {
    ...
    template <typename T> constexpr auto make_expected(T &&v) -> expected<typename std::decay<T>::type> {
