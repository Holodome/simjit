// This is modified version of span-lite to wrap it into simjit namespace instead of nonstd
...
public:
    ...
    template <
        typename It span_REQUIRES_T((std::is_convertible<decltype(*std::declval<It &>()), element_type &>::value))>
...
#else
        if (!detail::is_positive(idx) || size() <= idx) { detail::throw_out_of_range(idx, size()); }
        return *(data() + idx);
#endif
    }
#endif
...
template <class T> inline span_constexpr span<const T> make_span(std::initializer_list<T> il) span_noexcept {
